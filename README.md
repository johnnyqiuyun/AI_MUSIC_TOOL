...
1. 双击 `start.bat`（自动打开浏览器，并打印**局域网访问地址**发给同事）
2. 在网页上「上传歌曲」（WAV/FLAC/MP3，≤500MB），或把文件放进仓库**上级目录**作为共享曲库（可用环境变量 `STEM_STUDIO_SRC` 改路径）
3. 点「开始分离」，等几分钟（GPU 加速，多人同时用会自动排队），完成后自动进入混音台
4. 调好 M（静音）/ S（独奏）/ 推子后点「导出混音 WAV」

## 局域网多人使用

- 服务监听 `0.0.0.0:8765`，同一网络的同事直接用 `http://<你的IP>:8765` 打开
- 首次需要放行 Windows 防火墙（管理员命令行执行一次）：
  `netsh advfirewall firewall add rule name="Stem Studio" dir=in action=allow protocol=TCP localport=8765`
- 每个浏览器自动获得独立空间：**只能看到自己上传的歌**；放在服务器曲库目录里的歌所有人可见
- GPU 一次只处理一首，其余任务自动排队并显示队列位置；服务重启后未完成的排队任务自动恢复
- 不在同一局域网的人：装 [Tailscale](https://tailscale.com)（免费）组虚拟局域网即可访问，无需公网 IP
