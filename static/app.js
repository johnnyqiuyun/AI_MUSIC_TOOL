...
async function loadFiles() {
  ...
  try {
    ...
    list.innerHTML = `<div class="file-empty">无法连接后端服务，请确认 start.bat 已启动</div>`;
    return;
  }
  if (!files.length) {
    list.innerHTML = `<div class="file-empty">J:\\WAV 目录下没有找到 WAV 文件，放入歌曲后刷新页面</div>`;
    return;
  }
...
async function startSeparate(file, force) {
  const res = await fetch("/api/separate", {
    ...
    body: JSON.stringify({ file: file.name, force }),
  });
...
function watchProgress(jobId, songName) {
  ...
  const poll = async () => {
    let st;
    try {
      st = await (await fetch(`/api/job/${encodeURIComponent(jobId)}/status`)).json();
    } catch (e) { pollTimer = setTimeout(poll, 2000); return; }
    $("#progress-stage").textContent = st.stage || "";
...
async function doExport() {
  ...
  try {
    const res = await fetch(`/api/job/${encodeURIComponent(mixer.jobId)}/export`, {
      method: "POST", headers: { "Content-Type": "application/json" },
